...
```bash
cd server/
uv run python ../.Codex/skills/run-recordoc-be/smoke.py
```
...
```bash
uv run python ../.Codex/skills/run-recordoc-be/smoke.py 8001
```
...
- **`smoke.py` path is relative to `server/`.** The script resolves `SERVER_DIR` from its own `__file__`, so it must be run with `uv run python ../.Codex/...` from inside `server/`. Running it from the repo root will miscalculate `SERVER_DIR`.
